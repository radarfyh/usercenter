                ...
                if (body instanceof Flux) {
                    ...
                    return super.writeWith(fluxBody.buffer().map(dataBuffers -> {
                        ...
                        if (Objects.nonNull(contentType) && LogHelper.isUploadFile(contentType)) {
                            ...
                            log.info("ServerHttpResponseDecorator --> path:{},originalContent:{},content:{}", path, responseBody, content);
                            // 报文结构转换
                            Log.TYPE logType = Log.TYPE.RESPONSE;

                            content = messageTransformFactory.compareAndTransform(logType, path, responseBody, 
                            		content, charset, newResponseBody);
